...
*/
select concat_ws(' ',first_name,last_name) as Имя_фамилия from customer c ;
select email as Почта from customer c ; 
select char_length(email) as Длина from customer c ;
...
*/
select split_part(email, '@', 1) as Покупатель,
...
select initcap(split_part(email, '@', 1)) as Покупатель,
initcap(split_part(email, '@', 2)) as Домен
from customer c ; 
select 
    upper(substring(split_part(email, '@', 1) from 1 for 1)) ||
    lower(substring(split_part(email, '@', 1) from 2)) as Покупатель,
